 ...
 * @author Yvonne Wang
 */
class IsJListSelectedIndexTask extends GuiQuery<Boolean> {

  private final JList list;
  private final int index;

  static boolean isSelectedIndex(JList list, int index) {
    return new IsJListSelectedIndexTask(list, index).run();
  }

  private IsJListSelectedIndexTask(JList list, int index) {
    this.list = list;
    this.index = index;
  }
