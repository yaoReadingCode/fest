 ...
 * @author Yvonne Wang
 */
class GetJTableCellEditorComponentTask extends GuiTask<Component> {
  private final JTable table;
  private final int row;
  private final int column;

  static Component cellEditorComponentOf(JTable table, int row, int column) {
    return new GetJTableCellEditorComponentTask(table, row, column).run();
  }
  
  private GetJTableCellEditorComponentTask(JTable table, int row, int column) {
    this.table = table;
    this.row = row;
    this.column = column;
  }
