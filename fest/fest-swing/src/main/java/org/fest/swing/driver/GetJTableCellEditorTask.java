 ...
 * @author Yvonne Wang
 */
class GetJTableCellEditorTask extends GuiQuery<Component> {
  private final JTable table;
  private final int row;
  private final int column;

  static Component cellEditorIn(JTable table, int row, int column) {
    return new GetJTableCellEditorTask(table, row, column).run();
  }

  private GetJTableCellEditorTask(JTable table, int row, int column) {
    this.table = table;
    this.row = row;
    this.column = column;
  }
