...
public final class ScreenshotOnFailureResultFormatter extends XmlJUnitResultFormatter {
  ...
  private static final String FEST_OUTPUT_PROPERTY_NAME = "fest.output";
  ...
  private String festOutputFrom(JUnitTest suite) {
    return suite.getProperties().getProperty(FEST_OUTPUT_PROPERTY_NAME);
  }

  private void logFestOutput() {
    if (isEmpty(output)) {
      logEmptyFestOutput();
      return;
    } 
