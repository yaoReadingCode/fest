...
public class ComponentForegroundQueryTest {
  ...
  private ComponentForegroundQuery query;

  @BeforeMethod public void setUp() {
    component = createMock(Component.class);
    foreground = BLUE;
    query = new ComponentForegroundQuery(component);
  }
  
  public void shouldReturnComponentForeground() {
    new EasyMockTemplate(component) {
      protected void expectations() {
        expect(component.getForeground()).andReturn(foreground);
      }

      protected void codeToTest() {
        assertThat(query.executeInEDT()).isSameAs(foreground);
      }
    }.run();
  }
}
