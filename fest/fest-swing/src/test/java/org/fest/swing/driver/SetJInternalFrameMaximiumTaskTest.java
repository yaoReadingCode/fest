...
/**
 * Tests for <code>{@link SetJInternalFrameMaximumTask}</code>.
 *
 * @author Yvonne Wang
 */
@Test public class SetJInternalFrameMaximiumTaskTest {
  ...
  public void shouldCloseJInternalFrame() {
    new EasyMockTemplate(internalFrame) {
      ...
      protected void codeToTest() {
        try {
          new SetJInternalFrameMaximumTask(internalFrame, action).execute();
        } catch (PropertyVetoException e) {}
      }
    }.run();
  }
