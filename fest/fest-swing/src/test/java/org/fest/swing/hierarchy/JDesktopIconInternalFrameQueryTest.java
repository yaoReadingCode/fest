...
public class JDesktopIconInternalFrameQueryTest {
  ...
  private JDesktopIconInternalFrameQuery query;
  
  @BeforeMethod public void setUp() {
    desktopIcon = createMock(JDesktopIcon.class);
    internalFrame = createMock(JInternalFrame.class);
    query = new JDesktopIconInternalFrameQuery(desktopIcon);
  }
  
  public void shouldReturnInternalFrameOfJDesktopIcon() {
    new EasyMockTemplate(desktopIcon) {
      protected void expectations() {
        expect(desktopIcon.getInternalFrame()).andReturn(internalFrame);
      }

      protected void codeToTest() {
        assertThat(query.executeInEDT()).isSameAs(internalFrame);
      }
    }.run();
  }
}
