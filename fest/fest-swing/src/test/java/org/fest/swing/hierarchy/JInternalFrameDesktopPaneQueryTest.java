...
public class JInternalFrameDesktopPaneQueryTest {
  ...
  private JInternalFrameDesktopPaneQuery query;
  
  @BeforeMethod public void setUp() {
    internalFrame = createMock(JInternalFrame.class);
    desktopIcon = createMock(JDesktopIcon.class);
    desktopPane = createMock(JDesktopPane.class);
    query = new JInternalFrameDesktopPaneQuery(internalFrame);
  }
  
  public void shouldReturnNullIfJDesktopIconInJInternalFrameIsNull() {
    new EasyMockTemplate(internalFrame, desktopIcon) {
      protected void expectations() {
        expect(internalFrame.getDesktopIcon()).andReturn(null);
      }

      protected void codeToTest() {
        assertThat(query.executeInEDT()).isNull(); 
      }
    }.run();
  }
  ...
  public void shouldReturnJDesktopPaneFromJDesktopIconInJInternalFrameIsNull() {
    new EasyMockTemplate(internalFrame, desktopIcon) {
      protected void expectations() {
        ...
        expect(desktopIcon.getDesktopPane()).andReturn(desktopPane);
      }

      protected void codeToTest() {
        assertThat(query.executeInEDT()).isSameAs(desktopPane); 
      }
    }.run();
  }
}
