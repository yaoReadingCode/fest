...
/**
 * Understands a listener that displays all entries when on a web feed node of a <code>{@link WebFeedTree}</code> is
 * selected.
 * 
 * @author Alex Ruiz
 */
class WebFeedTreeWebFeedSelectionListener {

  static void attachTo(WebFeedTree tree) {
    tree.addTreeSelectionListener(new Listener(tree));
  }

  private WebFeedTreeWebFeedSelectionListener() {}
  ...
  private static class Listener implements TreeSelectionListener {
    ...
    public void valueChanged(TreeSelectionEvent e) {
      ...
      WebFeedNode webFeedNode = (WebFeedNode)selection;
      tree.mainFrame().displayEntriesOf(webFeedNode.webFeed);
    }
  }
}
